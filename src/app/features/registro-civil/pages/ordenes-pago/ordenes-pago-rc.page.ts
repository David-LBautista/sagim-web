...
@Component({
  ...
  templateUrl: './ordenes-pago-rc.page.html',
  styleUrls: ['./ordenes-pago-rc.page.scss'],
})
export class OrdenesPagoRcPage implements OnInit, OnDestroy {
  ...
  readonly areaResponsable = 'Registro Civil';
  readonly categoriaServicio = 'Registro Civil';
  ...
  private setupWebSocket(): void {
    console.log('[RC] setupWebSocket() — suscribiendo a ordenPagada$');
    this.wsService.ordenPagada$
      .pipe(takeUntil(this.destroy$))
      .subscribe((evento) => {
        console.log('[RC] orden:pagada recibido', evento);
        if (evento.area && evento.area !== this.areaResponsable) {
          console.log('[RC] ignorando evento de área:', evento.area);
          return;
        }
    ...
    // Refrescar tabla si el socket se reconectó (pueden haber llegado eventos perdidos)
    this.wsService.reconectado$.pipe(takeUntil(this.destroy$)).subscribe(() => {
      console.log('[RC] reconectado — refrescando órdenes');
      this.cargarOrdenes();
    });
  }
  // ── Carga ─────────────────────────────────────────────────────────────────
  ...
  onNuevaOrden(): void {
    ...
    ref.afterClosed().subscribe((orden: OrdenInterna | null | undefined) => {
      console.log('[RC] dialog cerrado, orden:', orden);
      this.cargarOrdenes();
    });
  }
